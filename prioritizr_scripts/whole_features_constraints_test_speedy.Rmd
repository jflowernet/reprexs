...
```{r}
#try creating boundary matrix based on Jeff's recommendation of creating an sf object from all pu data


pu_grid_sf <- rasterToPolygons(pu_raster) %>% 
  st_as_sf() %>% 
  select(-layer)

seamounts_sf <- rasterToPolygons(seamount_raster, dissolve = TRUE) %>% 
  st_as_sf() %>% 
  st_cast(to = "POLYGON") %>%
  select(-layer) %>% 
  #reversing order to get the same order as terra::patches() uses to number patches (starting top left working along each row left to right)
  {.[rev(rownames(.)),]}



#check that seamounts are numbered in the right order
mypal <- tmaptools::get_brewer_pal("Set1", n = 9)

plot(rownames_to_column(seamounts_sf)["rowname"], pal = mypal)
...
pu_sf <- bind_rows(pu_grid_sf, seamounts_sf)
...
prob <-
  ...
  add_boundary_penalties(penalty = 0.5, data = bnd_mat)
```
